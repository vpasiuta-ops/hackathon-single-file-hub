    ...
    if (action === 'createUser') {
      ...
      if (authData.user) {
        const profileData = {
          ...
          roles: userData.roles ? userData.roles.split(',').map((r: string) => r.trim()) : [],
          skills: userData.skills ? userData.skills.split(',').map((s: string) => s.trim()) : [],
          technologies: userData.technologies ? userData.technologies.split(',').map((t: string) => t.trim()) : [],
          experience_level: userData.experience_level,
          portfolio_url: userData.portfolio_url || '',
          bio: userData.bio || '',
          location: userData.location || '',
          ready_to_lead: userData.ready_to_lead,
          interested_categories: userData.interested_categories ? userData.interested_categories.split(',').map((c: string) => c.trim()) : [],
          existing_team_name: userData.existing_team_name || '',
          looking_for_roles: userData.looking_for_roles ? userData.looking_for_roles.split(',').map((r: string) => r.trim()) : [],
